Function Invoke-WindowsSecurityScript {

    <#
    .SYNOPSIS

        Invoke-WindowsSecurityScript - Checks and hardens your Windows configuration


         =^._.^=
        _(      )/  WindowsSecurityScript
    ...
    .DESCRIPTION
        ...
        WindowsSecurityScript supports hardening of a Windows system. The configuration of the system is
        retrieved and assessed using a finding list. In addition, the system can be hardened according
        to predefined values. WindowsSecurityScript reads settings from the registry and uses other modules
        to read configurations outside the registry.


    .PARAMETER FileFindingList

        Path to a finding list in CSV format. WindowsSecurityScript has one list each for machine and user settings.


    .PARAMETER Mode

        The mode Config only retrieves the settings, while the mode Audit performs an assessment of the settings.
        The mode HailMary hardens the system according to recommendations of the WindowsSecurityScript list.
    ...
    .PARAMETER Log
        ...
        name is assigned by WindowsSecurityScript itself and the file is stored in the same directory as the script.
    ...
    .PARAMETER Report
        ...
        The file name is assigned by WindowsSecurityScript itself and the file is stored in the same directory as the script.
    ...
    .PARAMETER SkipRestorePoint
        ...
        Do not create a System Restore Point in HailMary mode. WindowsSecurityScript strongly recommends to backup your system before running Hail Mary. However,
        ...
        Another reason is when WindowsSecurityScript is run as a user and thus lacks privileges.
    ...
    .EXAMPLE
        ...
        WindowsSecurityScript performs an audit, saves the results and creates a log file

    .EXAMPLE
        Invoke-WindowsSecurityScript -FileFindingList finding_list_0x6d69636b_user.csv -SkipMachineInformation

        WindowsSecurityScript performs an audit with a specific list and does not show machine information

    .EXAMPLE
        Invoke-WindowsSecurityScript -Mode Config -Report -ReportFile C:\tmp\my_WindowsSecurityScript_report.csv

        WindowsSecurityScript uses the default list, and saves the results in a specific file
    ...
    # Start Main
    #
    $WindowsSecurityScriptVersion = "0.9.3-1726808773"
    ...
    If ($Log) {
        If ($LogFile.Length -eq 0) {
            $LogFile = "WindowsSecurityScript_log_" + $Hostname + "_" + $ListName + "-$FileDate.log"
    ...
    If ($Report) {
        If ($ReportFile.Length -eq 0) {
            $ReportFile = "WindowsSecurityScript_report_" + $Hostname + "_" + $ListName + "-$FileDate.csv"
    ...
    If ($Backup) {
        If ($BackupFile.Length -eq 0) {
            $BackupFile = "WindowsSecurityScript_backup_" + $Hostname + "_" + $ListName + "-$FileDate.csv"
    ...
    Write-Output "`n"
    Write-Output "      =^._.^="
    Write-Output "     _(      )/  WindowsSecurityScript $WindowsSecurityScriptVersion"
    Write-Output "`n"
    Write-ProtocolEntry -Text "Starting WindowsSecurityScript" -LogLevel "Info"
    ...
    If ($WinSystemLocale.Name -ne "en-US" -and -not($SkipLanguageWarning)) {
        ...
        $Message = "WindowsSecurityScript was developed for the system language 'en-US'. This system uses '" + $WinSystemLocale.Name + "' Language-dependent analyses can sometimes produce false results. Please create an issue if this occurs."
    ...
    If ($Mode -eq "Audit" -or $Mode -eq "Config") {
        ...
        # A CSV finding list is imported. WindowsSecurityScript has one machine and one user list.
        If ($FileFindingList.Length -eq 0) {
        ...
        If ($Filter) {
            ...
            If ($FindingList.Length -eq 0) {
                $Message = "Your filter did not return any results, please adjust the filter so that WindowsSecurityScript has something to work with."
                Write-ProtocolEntry -Text $Message -LogLevel "Error"
                Break
            }
        }
        ...
        ForEach ($Finding in $FindingList) {
            ...
            # to a file, which means WindowsSecurityScript must create a temporary file
            # and afterwards delete it. WindowsSecurityScript is very orderly.
            #
            ...
            ElseIf ($Finding.Method -eq 'auditpol') {
                ...
                try {
                    ...
                    $TempFileName = [System.IO.Path]::GetTempPath() + "WindowsSecurityScript_auditpol-" + $(Get-Date -Format yyyyMMdd-HHmmss) + ".csv"
    ...
    #
    # Start HailMary mode
    # WindowsSecurityScript configures all settings in a finding list file.
    # Even though WindowsSecurityScript works very carefully, please only
    ...
    Elseif ($Mode -eq "HailMary") {
        ...
        If (-not($SkipRestorePoint)) {
            ...
            Try {
                ...
                Checkpoint-Computer -Description 'WindowsSecurityScript' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop -WarningAction Stop
            } catch {
        ...
        ForEach ($Finding in $FindingList) {
            ...
            If ($Finding.Method -eq 'Registry' -or $Finding.Method -eq 'RegistryList') {
                ...
                If ($Finding.RegistryItem -eq "ASRRules" -Or $Finding.RegistryItem -eq "ASROnlyExclusions" -Or $Finding.RegistryItem -eq "ExcludedExtensions" -Or $Finding.RegistryItem -eq "ExcludedPaths" -Or $Finding.RegistryItem -eq "ExcludedProcesses") {
                    $ResultText = "This setting is not configured by WindowsSecurityScript"
    ...
    # Start GPO mode
    # WindowsSecurityScript configures all settings in a finding list file.
    # Even though WindowsSecurityScript works very carefully.
    ...
    Write-Output "`n"
    Write-ProtocolEntry -Text "WindowsSecurityScript is done" -LogLevel "Info"
    ...
    If ($Mode -eq "Audit") {
        ...
        If ($ScoreTotal -ne 0 ) {
            $WindowsSecurityScriptScore = ([int] $ScoreAchived / [int] $ScoreTotal) * 5 + 1
        }
        $WindowsSecurityScriptScoreRounded = [math]::round($WindowsSecurityScriptScore, 2)

        # Overwrite WindowsSecurityScript Score if no finding is passed
        If ($StatsPassed -eq 0 ) {
            $WindowsSecurityScriptScoreRounded = 1.00
        }

        If ($Script:StatsError -gt 0) {
            Write-ProtocolEntry -Text "During the execution of WindowsSecurityScript errors occurred due to missing admin rights or tools. For a complete result, these errors should be resolved. Total errors: $Script:StatsError" -LogLevel "Error"
        }

        Write-ProtocolEntry -Text "Your WindowsSecurityScript score is: $WindowsSecurityScriptScoreRounded. WindowsSecurityScript Statistics: Total checks: $StatsTotal - Passed: $StatsPassed, Low: $StatsLow, Medium: $StatsMedium, High: $StatsHigh." -LogLevel "Info"
    }
    Write-Output "`n"
}

Export-ModuleMember -Function Invoke-WindowsSecurityScript
