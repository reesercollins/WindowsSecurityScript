...
This is the stable version of _WindowsSecurityScript_ from the [Windows Hardening Project by Michael Schneider](https://github.com/0x6d69636b/windows_hardening). The stable version of _WindowsSecurityScript_ is signed with the code signing certificate of _scip AG_. **Since this is the stable version, we do not accept pull requests in this repo, please send them to the [development repo](https://github.com/0x6d69636b/windows_hardening)**. 

_WindowsSecurityScript_ supports hardening of a Windows system. The configuration of the system is retrieved and assessed using a finding list. In addition, the system can be hardened according to predefined values. _WindowsSecurityScript_ reads settings from the registry and uses other modules to read configurations outside the registry.
...
Download _WindowsSecurityScript_ and copy it to the target system (script and lists). Then WindowsSecurityScript can be imported and executed:

```powershell
PS C:\tmp> Import-Module .\WindowsSecurityScript.psm1
PS C:\tmp> Invoke-WindowsSecurityScript -EmojiSupport


         =^._.^=
        _(      )/  WindowsSecurityScript 0.9.0-1662273740


[*] 9/4/2022 8:54:12 AM - Starting WindowsSecurityScript
...
[*] 9/4/2022 8:54:25 AM - WindowsSecurityScript is done
[*] 9/4/2022 8:54:25 AM - Your WindowsSecurityScript score is: 4.82. WindowsSecurityScript Statistics: Total checks: 325 - Passed: 213, Low: 33, Medium: 76, High: 3.
```

### How To Install

First create the directory *WindowsSecurityScript* and for every version a sub directory like *0.9.2* in a path listed in the *PSModulePath* environment variable.

Copy the module *WindowsSecurityScript.psm1*, *WindowsSecurityScript.psd1*, and the *lists* directory to this new directory.

```powershell
PS C:\tmp> $Version = "0.9.2"
PS C:\tmp> New-Item -Path $Env:ProgramFiles\WindowsPowerShell\Modules\WindowsSecurityScript\$Version -ItemType Directory
PS C:\tmp> Copy-Item -Path .\WindowsSecurityScript.psd1,.\WindowsSecurityScript.psm1,.\lists\ -Destination $Env:ProgramFiles\WindowsPowerShell\Modules\WindowsSecurityScript\$Version\ -Recurse
```
...
You can use the script below to download and install the latest release of *WindowsSecurityScript*.

```powershell
Function InstallWindowsSecurityScript() {
    ...
    $WindowsSecurityScriptLatestVersionDownloadLink = ((Invoke-WebRequest "https://api.github.com/repos/0x6d69636b/windows_hardening/releases/latest" -UseBasicParsing) | ConvertFrom-Json).zipball_url
    $ProgressPreference = 'SilentlyContinue'
    Invoke-WebRequest $WindowsSecurityScriptLatestVersionDownloadLink -Out WindowsSecurityScript$Version.zip
    Expand-Archive -Path ".\WindowsSecurityScript$Version.zip" -Destination ".\WindowsSecurityScript$Version" -Force
    $Folder = Get-ChildItem .\WindowsSecurityScript$Version | Select-Object Name -ExpandProperty Name
    Move-Item ".\WindowsSecurityScript$Version\$Folder\*" ".\WindowsSecurityScript$Version\"
    Remove-Item ".\WindowsSecurityScript$Version\$Folder\"
    New-Item -Path $Env:ProgramFiles\WindowsPowerShell\Modules\WindowsSecurityScript\$Version -ItemType Directory
    Set-Location .\WindowsSecurityScript$Version
    Copy-Item -Path .\WindowsSecurityScript.psd1,.\WindowsSecurityScript.psm1,.\lists\ -Destination $Env:ProgramFiles\WindowsPowerShell\Modules\WindowsSecurityScript\$Version\ -Recurse
    Import-Module "$Env:ProgramFiles\WindowsPowerShell\Modules\WindowsSecurityScript\$Version\WindowsSecurityScript.psm1"
}
InstallWindowsSecurityScript
```
...
The default mode is _audit_. WindowsSecurityScript performs an audit, saves the results to a CSV file and creates a log file. The files are automatically named and receive a timestamp. Using the parameters _ReportFile_ or _LogFile_, it is also possible to assign your own name and path.
...
```powershell
Invoke-WindowsSecurityScript -Mode Audit -Log -Report
```

WindowsSecurityScript can be executed with a specific list defined by the parameter _FileFindingList_. If WindowsSecurityScript is run several times on the same system, it may be useful to hide the machine information. The parameter _SkipMachineInformation_ is used for this purpose.

```powershell
Invoke-WindowsSecurityScript -FileFindingList .\lists\finding_list_0x6d69636b_user.csv -SkipMachineInformation
```

WindowsSecurityScript uses the default list, and checks only tests with the severity Medium.

```powershell
Invoke-WindowsSecurityScript -Filter { $_.Severity -eq "Medium" }
```

#### Config

The mode _config_ retrives all current settings of a system. If a setting has not been configured, WindowsSecurityScript will use a default value stored in the finding list. This mode can be combined with other functions, for example to create a backup.

WindowsSecurityScript gets the current settings and stores them in a report:

```powershell
Invoke-WindowsSecurityScript -Mode Config -Report -ReportFile C:\tmp\my_WindowsSecurityScript_report.csv
```

#### Backup

Backups are important. Really important. Therefore, WindowsSecurityScript also has a function to retrieve the current configuration and save it in a form that can be partially restored.

**Disclaimer:** WindowsSecurityScript tries to restore the original configuration. This works quite well with registry keys and Hardening Kitty really tries its best. But the backup function is not a snapshot and does not replace a real system backup. It is not possible to restore the system 1:1 with WindowsSecurityScript alone after HailMary. If this is a requirement, create an image or system backup and restore it.
...
```powershell
Invoke-WindowsSecurityScript -Mode Config -Backup
```
...
```powershell
Invoke-WindowsSecurityScript -Mode Config -Backup -BackupFile ".\myBackup.csv" -FileFindingList ".\list\{list}.csv"
```
...
```powershell
Invoke-WindowsSecurityScript -Mode HailMary -Log -Report -FileFindingList ".\myBackup.csv"
```
...
```powershell
Invoke-WindowsSecurityScript -Mode HailMary -Log -Report -FileFindingList .\lists\finding_list_0x6d69636b_machine.csv
```
...
The formula for the WindowsSecurityScript Score is _(Points achieved / Maximum points) * 5 + 1_.
...
## WindowsSecurityScript Interface

[ataumo](https://github.com/ataumo) build a web based interface for WindowsSecurityScript. The tool can be used to create your own lists and provides additional information on the hardening settings. The [source code](https://github.com/ataumo/policies_hardening_interface) is under AGPL license and there is a [demo site](https://phi.cryptonit.fr/policies_hardening_interface/).

### Last Update

WindowsSecurityScript can be used to audit systems against the following baselines / benchmarks:
